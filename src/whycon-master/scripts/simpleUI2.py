            ...
            while True:
                ...
                if (time.time()-last_slow_msg_time) >= SLOW_MSGS_LOOP_TIME:
                    ...
                    if next_msg == 'MSP_ANALOG':
                        ...
                        screen.addstr(17, 0, "ALTITUDE {}".format(ALTITUDE))
                        screen.clrtoeol()
                    ...
                    screen.clrtoeol()
                    screen.addstr(18, 0, "Throttle : {}".format(throttle))
                    screen.clrtoeol()
                    screen.addstr(19, 0, "Temp_Throttle : {}".format(tem_throttle))
                    screen.clrtoeol()
                    screen.addstr(20, 0, "PID : {}".format(gains))
                    screen.clrtoeol()
                    screen.addstr(21, 0, "Error : {}".format(error))
                    screen.clrtoeol()
                    screen.addstr(22, 0, "P : {}".format(P))
                    screen.clrtoeol()

                    screen.addstr(23, 0, "I : {}".format(I))
                    screen.clrtoeol()

                    screen.addstr(24, 0, "D : {}".format(D))
                    screen.clrtoeol()
...
class Controller():
    ...
    def pid(self,altitude,setpoints_=1.6):
        global throttle, tem_throttle, gains, error, P, I, D
        self.setpoints = setpoints_
        gains = [float(x) for x in pid]
        error = -(self.setpoints - round(altitude,1))
        
        if ARMED and altitude != 0:
            
            self.error_array.append(error)
            # if len(self.error_array) > 50:
            #     self.error_array.pop(0)
            # self.old_i = self.I


            P = gains[0] * error
            if tem_throttle < 1550 or tem_throttle > 1350:
                I = sum(self.error_array)*gains[1]
            D = gains[2]*(self.previous_error-error)
            tem_throttle = P + I + D
            # tem_throttle = self.P + self.I + self.D
            # temp_throttle = 
            throttle = tem_throttle
            if throttle >= 1550:
